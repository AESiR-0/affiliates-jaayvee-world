...
export function UpdatesPanel({ audience, apiBaseUrl = "https://talaash.thejaayveeworld.com" }: UpdatesPanelProps) {
  ...
  useEffect(() => {
    const fetchUpdates = async () => {
      try {
        setLoading(true);
        const url = `${apiBaseUrl}/api/updates?audience=${audience}`;
        ...
        console.error("API URL:", `${apiBaseUrl}/api/updates?audience=${audience}`);
        console.error("Response status:", err.response?.status || "N/A");
